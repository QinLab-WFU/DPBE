...
def get_args(main_args):
    ...
    parser.add_argument('-warm-up', default=20)
    parser.add_argument('--noise-rate', default=0)

    args = parser.parse_args()
    args = argparse.Namespace(**vars(args), **vars(main_args))

    args.save_dir = os.path.join(args.save_dir, args.method, args.dataset, str(args.output_dim), str(args.noise_rate))
