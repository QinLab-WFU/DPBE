...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, use_part=False):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, use_part=False):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
    ...
    def forward(self, x: torch.Tensor):
        ...
        # if method = PCME or MPC, annotation this process
        if self.proj is not None:
            cls = cls @ self.proj

        if self.use_part:
            part = self.ln_post(x[:, 1:, :])
            return cls, part
        else:
            return cls
...
def build_model(state_dict: dict, use_part=False):
    ...
    model = CLIP(
        ...
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, use_part
    )
